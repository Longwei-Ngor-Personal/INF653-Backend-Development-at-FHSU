...
switch ($action) {
    ...
    case "update_assignment":
        if ($assignment_id && $course_id && !empty($description)) {
            $success = update_assignment($assignment_id, $description, $course_id);
            if ($success) {
                header("Location: .?action=list_assignments&course_id=" . $course_id);
                exit();
            } else {
    ...
    case "update_course":  // ✅ NEW CASE FOR UPDATING COURSES
        if ($course_id && !empty($course_name)) {
            $success = update_course($course_id, $course_name);
            if ($success) {
                header("Location: .?action=list_courses");  // Redirect back to course list
                exit();
            } else {
